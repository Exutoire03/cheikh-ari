...
export default function AboutPageRedesigned() {
  return (
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-300 font-sans pt-28 pb-24 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">

        {/* --- SECTION 1: HÉROS / INTRODUCTION --- */}
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-center mb-32">
          <div className="lg:col-span-2 space-y-6">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-slate-900 dark:text-slate-100 tracking-tighter">
        ...
        <section className="mb-32">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-16 text-slate-900 dark:text-slate-100 tracking-tighter">Mon Parcours Professionnel</h2>
          <div className="relative max-w-2xl mx-auto">
            <div className="absolute left-4 top-0 h-full w-0.5 bg-gray-300 dark:bg-slate-800" aria-hidden="true"></div>
            
            <div className="space-y-12">
              {experiences.map((exp, index) => (
                <div key={index} className="relative pl-12">
                  <div className="absolute left-0 top-1.5 flex items-center justify-center w-8 h-8 rounded-full bg-gray-200 dark:bg-slate-800 border-2 border-sky-500 text-sky-500">
                    {exp.icon}
                  </div>
                  <div className="pl-4">
                    <p className="text-sm font-medium text-sky-500 dark:text-sky-400 mb-1">{exp.period}</p>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">{exp.title}</h3>
                    <p className="text-md text-slate-600 dark:text-slate-400 mb-3">{exp.company}</p>
        ...
        <section className="mb-32">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-16 text-slate-900 dark:text-slate-100 tracking-tighter">Compétences & Atouts</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {/* Compétences techniques */}
            {skills.map(skillGroup => (
              <div key={skillGroup.category} className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800
                                                    hover:border-sky-500/50 transition-colors duration-300">
                <div className="flex items-center gap-4 mb-4">
                  <span className="p-2 bg-gray-200 dark:bg-slate-800 rounded-lg text-sky-500 dark:text-sky-400">{skillGroup.icon}</span>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">{skillGroup.category}</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {skillGroup.technologies.map(tech => (
                    <span key={tech} className="px-3 py-1 bg-gray-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-full text-sm">{tech}</span>
                  ))}
                </div>
              </div>
            ))}
            {/* Soft Skills */}
            {softSkills.map(soft => (
              <div key={soft.skill} className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 
                                            hover:border-purple-500/50 transition-colors duration-300">
                <div className="flex items-center gap-4 mb-2">
                  <span className="text-2xl text-purple-500 dark:text-purple-400">{soft.icon}</span>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">{soft.skill}</h3>
                </div>
                <p className="text-slate-600 dark:text-slate-400 text-sm">{soft.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-16 text-slate-900 dark:text-slate-100 tracking-tighter">Au-delà du Code</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 text-center">
            {hobbies.map(hobby => (
              <div key={hobby.name} className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800
                                            flex flex-col items-center justify-center gap-3
                                            hover:-translate-y-1 transition-transform duration-300">
                <span className="text-4xl text-slate-700 dark:text-slate-200">{hobby.icon}</span>
                <p className="font-semibold text-slate-800 dark:text-slate-200">{hobby.name}</p>
              </div>
            ))}
          </div>
        </section>
